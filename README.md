...
CAS（Compare-and-Swap，比较并交换）是一种**硬件提供的原子操作**，作为现代并发编程的基石，它使多线程程序能够在不使用传统锁的情况下安全地修改共享数据。

### 核心机制

CAS操作包含三个参数：**内存位置（V）**、**预期原值（A）** 和 **新值（B）**。其核心思想是：**"我认为位置V的值应该是A，如果是，那么将其更新为B；否则，不进行任何修改，但请告诉我当前的实际值。"**

### 关键组件详解

1. **`compare_exchange_strong` 的作用**
   - **比较**：原子性地比较原子变量的当前值是否与预期值相等
   - **交换**：如果相等，则设置为新值，返回 `true`
   - **更新预期值**：如果不相等，将当前值写入预期值参数，返回 `false`
...
3. **内存顺序参数**
   - `std::memory_order_release`（成功时）：保证操作前的写入对其他线程可见
   - `std::memory_order_relaxed`（失败时）：仅保证原子性，性能最优
...
- CAS是实现无锁编程的基础构建块
- `std::atomic` 提供了线程安全的原子操作
- CAS循环模式确保复杂的读-改-写操作原子性
- 理解内存顺序对编写高性能并发代码至关重要
- 优先使用内置原子操作（如`fetch_add`），复杂场景使用CAS循环
